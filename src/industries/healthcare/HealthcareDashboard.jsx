  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'8px 12px',
      ...
      <div style={{ fontSize:20, fontWeight:700, color, fontFamily:'monospace', lineHeight:1 }}>
        {value != null ? (typeof value === 'number' ? Number(value).toFixed(0) : value) : '—'}
      </div>
...
function AssetCard({ asset, selected, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{
      background: selected ? 'rgba(239,68,68,0.08)' : isCritical ? 'rgba(239,68,68,0.04)' : 'rgba(255,255,255,0.03)',
      border: `1px solid ${selected ? 'rgba(239,68,68,0.5)' : isCritical ? 'rgba(239,68,68,0.25)' : 'rgba(255,255,255,0.07)'}`,
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:6 }}>
        <div>
          ...
          {asset.ward && (
            <span style={{ fontSize:10, color:'#64748b', marginLeft:6 }}>{asset.ward}</span>
          )}
        </div>
        {isCritical
          ? <span style={{ fontSize:10, fontWeight:700, color:'#ef4444',
              background:'rgba(239,68,68,0.12)', padding:'2px 8px', borderRadius:20 }}>CRITICAL</span>
...
function PatientDetail({ asset }) {
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(130px,1fr))', gap:10 }}>
      {fields.map(f => {
        const inRange = f.val != null && typeof f.val === 'number' && f.val >= f.low && f.val <= f.high;
        const color   = f.val == null ? '#64748b' : inRange ? '#22c55e' : '#ef4444';
        return (
          <div key={f.label} style={{ background:'rgba(255,255,255,0.03)',
            border:`1px solid ${color}30`, borderRadius:8, padding:'10px 12px' }}>
      ...
      {fields.map(f => (
        <div key={f.label} style={{ background:'rgba(255,255,255,0.03)',
          ...
          <div style={{ fontSize:18, fontWeight:700,
            color: f.label === 'Alarm Active' && f.val === 'YES' ? '#ef4444' : '#e2e8f0',
    ...
    <div style={{ padding:'24px 28px', minHeight:'100vh', background:'#0a0f1a', color:'#e2e8f0', fontFamily:'system-ui,sans-serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24 }}>
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:4 }}>
            ...
            <h1 style={{ fontSize:20, fontWeight:700, color:'#f1f5f9', margin:0 }}>Healthcare / Medical IoT</h1>
          </div>
          <div style={{ fontSize:12, color:'#475569' }}>
            Live vitals · {monitors.length} patients · {vents.length} ventilators monitored
          </div>
        </div>
      ...
      {critical > 0 && (
        <div style={{ marginBottom:16, padding:'10px 16px', borderRadius:8,
          ...
          🚨 <strong>{critical} patient{critical > 1 ? 's' : ''} in critical condition</strong> — Immediate attention required
        </div>
      )}

      <div style={{ display:'flex', gap:12, marginBottom:24, flexWrap:'wrap' }}>
        <KPICard label="Active Patients"    value={monitors.length}              color="#22c55e" />
        <KPICard label="Critical Status"    value={critical}                     color={critical > 0 ? '#ef4444' : '#22c55e'} />
        <KPICard label="Avg SpO2"           value={avgSpO2}         unit="%"     color={Number(avgSpO2) >= 95 ? '#22c55e' : '#ef4444'} />
        <KPICard label="Avg Heart Rate"     value={avgHR}           unit=" bpm"  color="#3b82f6" />
      ...
      <div style={{ display:'grid', gridTemplateColumns:'300px 1fr', gap:20, marginBottom:20 }}>
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          <div style={{ fontSize:12, fontWeight:600, color:'#64748b', textTransform:'uppercase',
            letterSpacing:'0.06em', marginBottom:4 }}>Patients & Devices ({assetList.length})</div>
          {assetList.length === 0 ? (
            <div style={{ textAlign:'center', padding:40, color:'#334155', fontSize:13,
              border:'1px dashed rgba(255,255,255,0.06)', borderRadius:10 }}>
              {status === 'connecting' ? 'Connecting…' : 'No devices. Start the simulator.'}
            </div>
          ) : (
            assetList.map(a => (
          ...
          <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.07)',
            ...
            {history.length < 2 ? (
              <div style={{ height:180, display:'flex', alignItems:'center', justifyContent:'center',
                color:'#334155', fontSize:12 }}>Waiting for vitals stream…</div>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={history} margin={{ top:5, right:10, bottom:5, left:0 }}>
                  ...
                  <Line type="monotone" dataKey="avgHR"   name="Avg HR (bpm)"  stroke="#ef4444" strokeWidth={2} dot={false} isAnimationActive={false}/>
                  <Line type="monotone" dataKey="avgSpO2" name="Avg SpO2 (%)"  stroke="#22c55e" strokeWidth={2} dot={false} isAnimationActive={false}/>
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
            ...
            <div style={{ background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.07)',
              ...
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:14 }}>
                <div style={{ fontSize:13, fontWeight:600, color:'#cbd5e1' }}>
                  ...
                  {selectedObj.ward && <span style={{ fontSize:11, color:'#64748b', marginLeft:6 }}>{selectedObj.ward}</span>}
